...
module.exports = {
	name: 'textclear',
	description: 'Start clear text loop',
	execute(message, args) {
        ...
        //Loop to continually delete text
        var timer = setInterval(function () {
            if (message.channel.type == 'text') {
                message.channel.messages.fetch().then(messages => {
                    const botMessages = messages.filter(msg => msg.author.bot);
                    const textMessages = messages.filter(msg => msg.embeds.length === 0 && msg.attachments.size === 0);
                    const cmdMessages = messages.filter(msg => msg.content === "$textclear");
        ...
        message.channel.send('Starting text cleanup loop...');
        console.log('Starting text cleanup loop...')
    },  
};
